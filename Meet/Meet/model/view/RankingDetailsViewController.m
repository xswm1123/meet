//
//  RankingDetailsViewController.m
//  Meet
//
...
@interface RankingDetailsViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation RankingDetailsViewController
...
-(void)loadDatas{
    ...
    if ([sortId isEqualToString:@"totalPay"]) {
        reqs.sortField=@"total_pay";
    }else{
         reqs.sortField=sortId;
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ...
        if (self.index==4) {
            cell.lb_desc.text=[NSString stringWithFormat:@"魅力值:%@",[self.myRank objectForKey:@"meili"]];
        }
        if (self.index==5) {
            cell.lb_desc.text=[NSString stringWithFormat:@"总消费:%@",[self.myRank objectForKey:@"totalPay"]];
        }
        if (self.index==6) {
            cell.lb_desc.text=[NSString stringWithFormat:@"总财富:%@",[self.myRank objectForKey:@"gold"]];
        }
        
        cell.lb_no.text=[NSString stringWithFormat:@"%@",[self.myRank objectForKey:@"ranking"]];
        return cell;
    }
    NSDictionary * dic=[self.rankList objectAtIndex:indexPath.section-1];
    ...
    if (self.index==4) {
        cell.lb_desc.text=[NSString stringWithFormat:@"魅力值:%@",[dic objectForKey:@"meili"]];
    }
    if (self.index==5) {
        cell.lb_desc.text=[NSString stringWithFormat:@"总消费:%@",[dic objectForKey:@"totalPay"]];
    }
    if (self.index==6) {
        cell.lb_desc.text=[NSString stringWithFormat:@"总财富:%@",[dic objectForKey:@"gold"]];
    }
    
    cell.lb_no.text=[NSString stringWithFormat:@"%d",indexPath.section];
    return cell;
}
