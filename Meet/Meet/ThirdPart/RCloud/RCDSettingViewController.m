...
-(void) setSwitchState
{
    ...
    //设置新消息通知状态
    [[RCIMClient sharedClient] getConversationNotificationStatus:self.conversationType targetId:self.targetId completion:^(RCConversationNotificationStatus nStatus) {
    ...
    //设置置顶聊天状态
    RCConversation *conversation = [[RCIMClient sharedClient] getConversation:self.conversationType targetId:self.targetId];
...
-(void)onClickClearMessageHistory:(id)sender
{
    ...
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"是否删除历史消息？" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"确定" otherButtonTitles:nil, nil];
